...
class LoginUsuario(APIView):
    def post(self, request):
        nombre_usuario = str(request.data.get('nombre_usuario') or '').strip()
        contrasena = str(request.data.get('contrasena') or '')
        ...
        usuario = Usuario.objects.filter(
            Q(nombre_usuario__iexact=nombre_usuario) | Q(correo__iexact=nombre_usuario)
        ).first()
        ...
        if not contrasena_valida and contrasena == contrasena_guardada:
