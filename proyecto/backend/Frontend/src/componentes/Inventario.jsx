      ...
      <div className="col-span-12 space-y-6 lg:col-span-4">
        ...
        <div className="rounded-xl border border-[#3f371a] bg-[#11110e] p-6">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            {[{ title: "Alertas de reposición", description: "Artículos que alcanzaron el umbral de reposición.", color: "border-[#ffd700]", text: "text-[#ffd700]", items: lowStockArticulos.filter((item) => item.nivel === "Reposición") }, { title: "Alertas críticas de stock", description: "Artículos por debajo del mínimo configurado o con menos de cinco unidades.", color: "border-[#ff594e]", text: "text-[#ff8f84]", items: lowStockArticulos.filter((item) => item.nivel === "Crítico") }].map((group) => <div key={group.title} className={`rounded-lg border-l-2 ${group.color} bg-[#0a0a0a] p-4`}><div className="mb-3"><h4 className={`text-sm font-black uppercase ${group.text}`}>{group.title}</h4><p className="mt-1 text-[11px] text-[#aaa79d]">{group.description}</p></div>{group.items.length === 0 ? <p className="text-sm text-[#6e6a5c]">No hay artículos en esta categoría.</p> : <div className="max-h-96 space-y-2 overflow-y-auto pr-1">{group.items.map((articulo) => <div key={articulo.id_articulo} className="flex items-center justify-between rounded border border-[#24231b] p-3"><div><strong className="text-sm text-[#e3e2e2]">{articulo.nombre_articulo}</strong><p className="text-[10px] text-[#aaa79d]">{articulo.tipo_articulo || "General"}</p></div><span className={`font-mono text-sm font-black ${group.text}`}>{articulo.stock_actual} uds</span></div>)}</div>}</div>)}
          </div>
        </div>
      </div>
